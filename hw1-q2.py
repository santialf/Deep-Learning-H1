...
class LogisticRegression(nn.Module):
    ...
    def __init__(self, n_classes, n_features, **kwargs):
        ...
        self.logits = nn.Linear(n_features, n_classes)
...
class FeedforwardNetwork(nn.Module):
    def __init__(
        ...
        # layers of our nn
        self.inLogits = nn.Linear(n_features, hidden_size)
        self.hiddenLogits = nn.ModuleList()
        # only create hidden logits when number of layers is bigger than 1
        for i in range(layers - 1):
            self.hiddenLogits.append(nn.Linear(hidden_size, hidden_size))
        self.outLogits = nn.Linear(hidden_size, n_classes)
    ...
    def forward(self, x, **kwargs):
        ...
        # hidden layers
        for i, layer in enumerate(self.hiddenLogits):
            x = layer(x)
            x = self.activation(x)
            x = self.dropout(x)

        # output layer
        return self.outLogits(x)
        #raise NotImplementedError
...
def train_batch(X, y, model, optimizer, criterion, **kwargs):
    ...
    loss.backward() 
    # updates weights using the gradients.
    optimizer.step() 
